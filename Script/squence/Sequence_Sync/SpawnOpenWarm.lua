---@class SpawnOpenWarm_C:LevelSequenceActor
--Edit Below--
...
function SpawnFirstBoss:ReceiveBeginPlay()
    ...
    local oBTimerDelegate1 = ObjectExtend.CreateDelegate(self,
        function()      
            ...
            local MonsterPath = UGCMapInfoLib.GetRootLongPackagePath().."Asset/Blueprint/NPC/WarmMachine.WarmMachine_C"
            local Class_Monster = UE.LoadClass(MonsterPath)
            local BP_Monster = ScriptGameplayStatics.SpawnActor(self, Class_Monster, 
                                    {X=53164.738281,Y=20168.513672,Z=-47.716751}, --坐标
    ...
    local path = UGCGameSystem.GetUGCResourcesFullPath('Asset/UI/Captions/OpenWarm.OpenWarm_C')
    ...
    -- local MonsterPath = UGCGameSystem.GetUGCResourcesFullPath('Asset/Blueprint/NPC/WarmMachine.WarmMachine_C')
    -- local Class_Monster = UE.LoadClass(MonsterPath)
    -- local BP_Monster = ScriptGameplayStatics.SpawnActor(self, Class_Monster, 
    --                            {X=13768.395508,Y=20218.513672,Z=-47.716751},    --坐标
    --                            {Roll = 0, Pitch = 0, Yaw = 0},     --旋转
    --                            {X = 1, Y = 1, Z = 1})              --缩放
