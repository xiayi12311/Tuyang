---@class BPWidget_Shop_C:UUserWidget
---@field NewAnimation_1 UWidgetAnimation
---@field Button_EquipConsume UButton
---@field Button_HeavyWeapon UButton
...
function BPWidget_Shop:Construct()
    ...
    UGCLog.Log("[YYH]BPWidget_Shop_TuYangConstruct0")
    UGCLog.Log("[YYH]BPWidget_Shop_TuYangConstruct00", #self.DataSource.Items)
    UGCLog.Log("[YYH]BPWidget_Shop_TuYangConstruct00", self.DataSource.Items[1].Belong)
    
    BPWidget_Shop.SuperClass.Construct(self)

    -- 自适应页数
   Category = 1
   for K, V in pairs(self.DataSource.Items) do
       UGCLog.Log("BPWidget_Shop_TuYangConstruct01",V.Belong)
...
function BPWidget_Shop:InvalidateDataSource()
    UGCLog.Log("[YYH]BPWidget_ShopInvalidateDataSource")
...
function BPWidget_Shop:AddPage()
    UGCLog.Log("[YYH]BPWidget_ShopAddPage",Page,PageMax[Category])
    if Page < PageMax[Category] then
        Page = Page + 1
    end    
end
function BPWidget_Shop:SubPage()
    if Page > 1 then
        Page = Page - 1
    end
end
